...
do
  ...
  case $var_name in
    ...
    "instance-private-nic2-ip")
      ...
      do
        COUNT=$(($COUNT+1))
        NODE_PRIVATE_NIC2_IP[$COUNT]=$entry
      done
      ;;
    ...
    "instance-private-nic3-ip")
      ...
      do
        COUNT=$(($COUNT+1))
        NODE_PRIVATE_NIC3_IP[$COUNT]=$entry
      done
      ;;
...
do
  echo ${NODE_NAME[$i]} ${NODE_PUBLIC_IP[$i]} ${NODE_PRIVATE_IP[$i]} ${NODE_PRIVATE_NIC2_IP[$i]} ${NODE_PRIVATE_NIC3_IP[$i]}
done
